// 훅스 useState 먼저 확인 복습 해보기 지금 부터 모두 함수형 컴포넌트로 작업
...
const InfoTestUseEffect = () => {
  ...
  // useEffect Test 하기. 정의 useEffect(콜백함수, 의존성 배열) 의존성 배열 모양 1) 아무것도 없을 때, 매번 콜백함수
  // 실행되고, 2) [], 빈 배열, 한번만 실행되고 3) [list], list의 상태가 변경 될 때 마다, 콜백함수 실행이 됨
  useEffect(() => {
    console.log("useEffect 호출됨.");
    console.log({ name, nickName });
    // 후처리 함수 추가하기 문법 :  return () => {수행할 로직}
    return () => {
  ...
  //결과 출력하기. 작성 문법은 JSX -> 기존 HTML 형식과 동일. 리액트 컴포넌트는 대문자 시작, 기존 DOM , 소문자 태그.
  return (
    <div>
      <Button
        onClick={() => {
      ...
      <div>{visible ? "visible: false" : "visible : true"}</div>
      <div>
